...
function Strategy(options, verify) {
  ...
  this._userTeamsURL = options.userTeamsURL || 'https://api.github.com/user/teams';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    self._oauth2.get(self._userTeamsURL, accessToken, function (err, body, res) {
      ...
      for (var index in json) {
          var team = {'name': json[index].name,
                      'org': json[index].organization.login};
          profile.teams.push(team);
      }

      if (res.rawHeaders.includes('Link')) {
        // Teams return more than one page
        for (var index in res.rawHeaders) {
          if (res.rawHeaders[index].includes('last')) {
            // Get last page number
            // <https://api.github.com/user/teams?page=X>; rel="next", <https://api.github.com/user/teams?page=X>; rel="last"
            var page = res.rawHeaders[index].split("=")[3].split(">")[0];
            break;
          }
        }
        var task = page;
        for (var i = 2; i <= page; i++) {
          var teamsURL = self._userTeamsURL + "?page=" + i;
          self._oauth2.get(teamsURL, accessToken, function (err, body, res) {
            var json = JSON.parse(body);
            for (var index in json) {
              var team = {'name': json[index].name,
                          'org': json[index].organization.login};
